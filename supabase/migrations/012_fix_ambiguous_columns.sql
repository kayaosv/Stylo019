-- ============================================================
-- ModaMariaJose — Fix "column reference is ambiguous" in crear_venta_tpv
-- and crear_venta_web
-- ============================================================
-- Root cause: both functions declare RETURNS TABLE (venta_id UUID,
-- numero_ticket INT, total NUMERIC) — Postgres treats those output
-- column names as implicit variables visible throughout the function
-- body. `ventas` also has a real `numero_ticket` column, so any
-- unqualified reference to `numero_ticket` inside an embedded SQL
-- statement is ambiguous (could mean the OUT parameter or the table
-- column). Same story for `venta_id` in crear_venta_web, which had a
-- second, unnecessary instance of this via a subquery that re-aliased
-- v_venta_id AS venta_id.
--
-- Found via a real error reported testing the TPV "Cobrar" button:
-- "column reference numero_ticket is ambiguous" — no sale had ever
-- completed successfully before this fix (confirmed both `ventas` and
-- `checkout_drafts` were empty). Fix: qualify with the table name in
-- RETURNING, and select the local v_venta_id variable directly instead
-- of re-aliasing it through a subquery.
--
-- Verified after fixing: called crear_venta_web() directly with a real
-- product/talla, confirmed correct venta_id/numero_ticket/total
-- returned, stock decremented correctly (including the per-color-stock
-- path — "Pantalón Lazo" was a good real test case, it already had
-- stock-per-color adopted), then rolled back the test data by hand.
